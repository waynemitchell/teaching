...
% Slide TODO: put this somewhere else 
% \begin{frame}{Preliminaries and a model problem}
% \begin{block}{Properties of SPD matrices}
% \bit
% \item Define an $A$ inner product and norm: 
% \eq{
...
% \item Search for a solution in the Krylov subspace, $\mathcal{K}_r(A,\mathbf{f}) = \text{span}\{\mathbf{f},A\mathbf{f},A^2\mathbf{f},...,A^{r-1}\mathbf{f}\}$ !!! TODO: check this? Notation and correctness...
% TODO: note that use of A inner product assumes an SPD matrix (CG only for SPD matrices!)
\eit
\end{block}
\end{frame}
...
\bit
\item Can be made more efficient etc. TODO
\eit
\end{block}
\end{frame}
...
\State $\mathbf{u} \leftarrow \mathbf{u} + \alpha\mathbf{p}_k$
\State $\mathbf{r}_{k+1} \leftarrow \mathbf{r}_k - A\mathbf{p}_k$
\EndFor
\end{algorithmic}
\end{algorithm}
\end{frame}
...
% TODO: mention GMRES and BiCGSTAB for non-symmetric
...
\eq{
   ...
   M^{-1}(\mathbf{f} - A\mathbf{u}) = \mathbf{0}
}
...
% TODO: better description of condition number? Something about residual vs. error from MG tutorial slides
